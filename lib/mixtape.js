...
const getPath = p => p.split('.').filter(Boolean).reverse()
  .reduce((a, k) => o => a(o && o[k]), pass)
...
exports.test = (...testArgs) => {
  ...
  const cache = {
    ...
    map: map => {
      stack[stack.length -1].map = (typeof map === 'string')
        ? getPath(map) : map
      return getter
    },
    ...
    start: (tester = tape) => {
      ...
      tapeArgs.push(opts, t => {
        ...
        Promise.resolve(fn()).catch(pass).then(actualValue => {
          stack.forEach(({ key, args, map=pass }) => noValue.has(key)
            ? t[key](...args)
