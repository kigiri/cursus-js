...
const preCli = new engine({
  ...
  rules: {
    ...
    'id-match': [ 'error', "^_?[a-z]+([A-Z][a-z]+)*_?$" ],
...
// guard this
// guard classes
// guard bind, call, apply
// guard function keyword


const exPath = `${__dirname}/../exercise`
module.exports = fs.readdir(exPath)
  .then(exercises => Promise.all(exercises
    .map(filename => fs.readFile(`${exPath}/${filename}`)
      .then(code => cli.executeOnText(code, filename)))))
